...
async def wishbirthday():
    ...
    for i in range(0, len(birthday_dodos)):
        ...
        await channel.send(f"Happy Birthday {username.display_name}/{username.name}!")
    c.close()
    db.close()
...
async def leaderboard(ctx):
    ...
    for i in range(0, 5):
        ...
        description_embed = description_embed + str(position) + ". " + username.display_name + " (" + username.name + "): " + str(money) + "\n"
